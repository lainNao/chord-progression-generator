export function convertAstToChordProgressionString(ast) {
    return ast
        .map((section) => {
        const sectionMetaInfos = section.metaInfos.length > 0
            ? section.metaInfos
                .map((metaInfo) => `@${metaInfo.type}=${metaInfo.value}`)
                .join("\n") + "\n"
            : "";
        const chords = section.chordBlocks
            .map((chordInfos) => chordInfos
            .map((chordInfo) => {
            const metaInfos = !!chordInfo.metaInfos.length
                ? `[${chordInfo.metaInfos
                    ...
                    .join(",")}]`
                : "";
            const chord = chordInfo.chordExpression.type === "noChord"
                ? "_"
